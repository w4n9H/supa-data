...
from supadata.core import prompt, AutoLLM, extract_code
from supadata.supatypes import SupaDataArgs, SourceCode


class TokenLimiter:
    def __init__(
        self, count_tokens: Callable[[str], int], full_text_limit: int, segment_limit: int, buff_limit: int,
        llm: AutoLLM, args: SupaDataArgs, disable_segment_reorder: bool,
    ):
    ...
    def limit_tokens(
        ...
        if len(final_relevant_docs) < len(reorder_relevant_docs):
            ...
            logger.info(f"第一轮 DocLimit, 首轮文档数:{len(self.first_round_full_docs)},剩余文档数:{len(remaining_docs)}")
            ...
            logger.info(f"第二轮 DocLimit,第二轮文档数:{self.second_round_extracted_docs},处理耗时:{self.sencond_round_time:.2f}秒")
